...
def _batch_decode(ids, timeout=12):
    if not ids:
        return []
    envelopes = []
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Mozilla/5.0 (compatible; ECRINTEL/2.4)",
        },
        method="POST",
    )
    with urllib.request.urlopen(req, timeout=timeout) as r:
        text = r.read().decode("utf-8", "ignore")
    found = re.findall(r'\[\\"garturlres\\",\\"(.*?)\\",', text)
    out = []
    for u in found:
